...
pub async fn get_all(db: &Db) -> Result<Vec<Slide>, diesel::result::Error> {
    let result = db
        .run(move |conn| slides::table.load::<(i32, String, String, Option<i32>, Option<String>, Option<i32>)>(conn))
        .await?;
    ...
    let slide: Slide = db
        .run(move |conn| {
            slides::table
                ...
                .first::<(i32, String, String, Option<i32>, Option<String>, Option<i32>)>(conn)
        })
        .await?
        .into();
    ...
    let slides = db
        .run(move |conn| {
            slides::table
                ...
                .load::<(i32, String, String, Option<i32>, Option<String>, Option<i32>)>(conn)
        })
        .await?
        .into_iter()
        .map(|s| s.into())
    ...
    let result = db
        .run(move |conn| {
            diesel::insert_into(slides::table)
                .values(&new_slide)
                .get_result::<(i32, String, String, Option<i32>, Option<String>, Option<i32>)>(conn)
        })
        .await?;
